...
#+TITLE: Lección 3. Modelos lineales
...
#+begin_abstract
Analizaremos las dificultades que plantea la *correlación serial* y veremos cómo la estacionariedad en sentido débil los simplifica. 
Los modelos de procesos estacionarios son útiles para representar la dependencia de los valores de una serie temporal de su pasado.
Nos centraremos en los *procesos lineales*, estudiando su valor esperado, su función de autocovarianza, las covarianzas cruzadas entre ellos.  
#+end_abstract

- ([[https://mbujosab.github.io/Econometria-Aplicada/Transparencias/Lecc03.slides.html][slides]]) --- ([[https://mbujosab.github.io/Econometria-Aplicada/Lecciones-html/Lecc03.html][html]]) --- ([[https://mbujosab.github.io/Econometria-Aplicada/Lecciones-pdf/Lecc03.pdf][pdf]]) --- ([[https://mybinder.org/v2/gh/mbujosab/Econometria-Aplicada/gh-pages?labpath=CuadernosElectronicos/Lecc03.ipynb][mybinder]])
...
1. Las series temporales se clasifican en *estacionarias* y *no estacionarias.*
...
#+attr_ipynb: (slideshow . ((slide_type . notes)))
#+BEGIN_SRC jupyter-python :results file :file ./img/lecc03/PIB_UEM.png :results silent 
...
[[./img/lecc03/PoblacionAustralia.png]]
...
#+attr_ipynb: (slideshow . ((slide_type . skip)))
#+BEGIN_SRC jupyter-python :results file :file ./img/lecc03/TemperaturaReriro.png :results silent
...
#+attr_html: :width 900px
#+attr_latex: :width 425px
[[file:./img/lecc03/TemperaturaRetiro.png]]

# [[./img/lecc03/TemperaturaReriro.png]]
...
#+attr_ipynb: (slideshow . ((slide_type . notes)))
#+BEGIN_SRC jupyter-python :results file :file ./img/lecc03/IBEX35.png :results silent
...
#+attr_ipynb: (slideshow . ((slide_type . notes)))
#+BEGIN_SRC jupyter-python :results file :file ./img/lecc03/ProduccionCemento.png :results silent
fig, ax = plt.subplots(figsize=(15, 5))
...
#+attr_html: :width 900px
#+attr_latex: :width 425px
[[./img/lecc03/ProduccionCemento.png]]
...
- En tres de ellas ---la población en Australia, el PIB de la zona euro y la producción de cemento--- se observa una tendencia clara, lo que implica una evolución de la media local a medio y largo plazo. 
...
#+attr_ipynb: (slideshow . ((slide_type . skip)))
Un proceso estocástico \(\boldsymbol{X}\) es un conjunto de variables aleatorias \(X_t\) donde el índice \(t\) toma valores en un cierto conjunto \(C\). En nuestro caso, este conjunto es el conjunto de los números enteros \(\mathbb{Z}\).
...
Cuando $\boldsymbol{X}$ es [[./Lecc01.slides.html#/3/1][débilmente estacionario]] se reduce /drásticamente/ el número de parámetros (aunque siga habiendo infinitos):
\begin{eqnarray}
  E(X_t)  = &  \mu \\
...
#+attr_ipynb: (slideshow . ((slide_type . skip)))
Un espacio vectorial, dotado de producto escalar y norma; por ejemplo $$\langle X \mid Y
...
\end{align*}
#+latex: \noindent
(la secuencia solo contiene la covarianza de un orden $k$ fijo... pero en distintos instantes $t$).
#+latex: \medskip
...
recorre todos los números enteros (son muchos momentos). _Por
...
Un proceso estocástico de segundo orden $\boldsymbol{X}$ se dice que es /débilmente estacionario/ @@latex:(estacionario en covarianza o, sencillamente, estacionario)@@ si:
- $E(X_t)=\mu$ para todo $t$ y 
- la covarianza entre $X_s$ y $X_t$ solo depende de la diferencia $s-t$.
# para todo $s,t\in\mathbb{Z}$.

En tal caso, llamamos _función de autocovarianzas_ a la siguiente secuencia:
\[
 \boldsymbol{\gamma} 
 \; = \; 
...
#+BEGIN_EXPORT latex
\noindent
(nótese que esta secuencia sí incluye covarianzas para \underline{todos} los órdenes $k\in\mathbb{Z}$).
\medskip
...
Tanto la secuencia $\boldsymbol{\gamma}$ como la matriz $\boldsymbol{\Gamma}$ son \textbf{definidas positivas}; es decir, para todos los enteros $n\geq1$ y escalares $c_1,c_2,\ldots,c_n$
...
Y@@latex:, si $\gamma_0>0$,@@ llamamos _función de autocorrelación_ (ACF) a la secuencia:
...
# demás procesos (débilmente) estacionarios.
#+latex:\bigskip
...
& = \lim_{m,n\to\infty} \left\langle \sum_{i=-m}^m {b}_i U_{t+k-i}\mid \sum_{j=-n}^n {b}_j U_{t-j}\right\rangle \nonumber \\
...
& = \sigma^2 \sum_{i=-\infty}^\infty {b}_i {b}_{i+k} \;=\; \sigma^2 \Big(\boldsymbol{b}(z)*\boldsymbol{b}(z^{-1})\Big)_k
\label{eq:FAC_proceso_lineal}
\end{align}
que solo depende de $k$ (en la tercera línea, $\delta_{p,q}$ es la delta de Kronecker; y en la cuarta  hemos usado la última ecuación de la Lección 2).
\medskip
...
Sea $\;\boldsymbol{U}\sim WN(0,\sigma^2)\;$ y sea $\;\boldsymbol{\psi}\in \ell^2\;$ una serie formal con _infinitos términos NO nulos_ y ${\color{#008000}{\psi_{0}=1}}$; 
...
Por ejemplo, cuando $\boldsymbol{\psi}$ tiene un número finito de términos no nulos...
...
\]
donde $\;\boldsymbol{\phi}\;$ un _polinomio de grado $(p)$_ con ${\color{#008000}{\phi_{0}=1}}$.
#+latex:\medskip
...
#+attr_ipynb: (slideshow . ((slide_type . fragment)))
Pero... ¿qué secuencia $\boldsymbol{a}$ usamos como inversa de $\boldsymbol{\phi}$?
...
1) _lineal y estacionario_, exigiremos que $\boldsymbol{\phi}$ _no tenga raíces de módulo 1_.
...
2) _causal_, exigiremos que las raíces de $\boldsymbol{\phi}$ sean mayores que 1 en valor absoluto (_raíces fuera del círculo unidad_):
...
#+attr_ipynb: (slideshow . ((slide_type . fragment)))
   (¡de nuevo un proceso lineal causal! En particular es un MA($\infty$) pues ${\color{#008000}{a_0=1}}$ por ser ${\color{#008000}{\phi_{0}=1}}$) 
#+latex:\bigskip
...
Sea $\;\boldsymbol{X}=\boldsymbol{\psi}*\boldsymbol{U},\;$ donde $\boldsymbol{\psi}$ es una serie formal de cuadrado sumable @@latex:($\boldsymbol{\psi}\in\ell^2$)@@ y donde $\;\boldsymbol{U}\sim WN(0,\sigma^2).\quad$ 
...
(demo en el [[https://mbujosab.github.io/Econometria-Aplicada/Lecciones-pdf/Lecc03.pdf#subsection.4.2][pdf]]@@latex: ---ecuación \eqref{eq:FAC_proceso_lineal}@@):
\[
 \gamma_{_{k,t}}
 \;=\;
...
# Es más, por la [[https://mbujosab.github.io/Econometria-Aplicada/Lecciones-pdf/Lecc02.pdf#section.7][última ecuación de la lección 2]] 
# \[
#  \gamma_k
#  \;=\;
#  \sigma^2\sum\nolimits_{j\in\mathbb{Z}}\psi_{j+k}\psi_j
...
#jupyter nbconvert --execute --inplace Lecc03.ipynb
#+END_SRC

#+BEGIN_SRC sh :results silent
#jupyter nbconvert --config ../mycfg-GitHubPages.py --to slides --reveal-prefix "https://unpkg.com/reveal.js@5.2.1" --execute Lecc03.ipynb
#+END_SRC
