...
#+TITLE: Lección 6. Modelos ARMA. Herramientas estadísticas
...
# +LATEX_HEADER: \PassOptionsToPackage{table}{xcolor}
...
- ([[https://mbujosab.github.io/Econometria-Aplicada/Transparencias/Lecc06.slides.html][slides]]) --- ([[https://mbujosab.github.io/Econometria-Aplicada/Lecciones-html/Lecc06.html][html]]) --- ([[https://mbujosab.github.io/Econometria-Aplicada/Lecciones-pdf/Lecc06.pdf][pdf]]) --- ([[https://mybinder.org/v2/gh/mbujosab/Econometria-Aplicada/gh-pages?labpath=CuadernosElectronicos/Lecc06.ipynb][mybinder]])
...
** Proceso autorregresivo ARMA(p,q)
   ...
   :ID:       61d1ee85-d45b-4c23-8c56-d0c68773a4d0
   :END:
...
/autorregresivo/ $\;\boldsymbol{\phi}\;$ tiene _grado_ $p$ con
${{\phi_{0}=1}}$, y
el polinomio /de media móvil/ $\;\boldsymbol{\theta}\;$ es de _grado_
$q$ con ${{\theta_{0}=1}};\;$ y donde
$\boldsymbol{\phi}$ y $\boldsymbol{\theta}$ tienen sus raíces fuera del círculo unidad (y ninguna es común; i.e., /no se pueden cancelar/).
# +latex:\medskip
...
#+attr_ipynb: (slideshow . ((slide_type . skip)))
#+latex: \noindent
La sección [[id:315309b8-425e-48eb-a97d-083940275fe0][Función de autocovarianzas para un ARMA(p,q)]] muestra que en un proceso ARMA($p,q$):
...
Consecuentemente, en cuanto a la ACF $\boldsymbol{\rho}$:
 ...
 - Los $p$ valores de $\rho_q$ a $\rho_{q-p+1}$ son los valores
   iniciales para resolver la ecuación en diferencias de [[https://mbujosab.github.io/Econometria-Aplicada/Lecciones-pdf/Lecc05.pdf#section.5][Yule-Walker]]
...
# +LATEX: \rowcolors{2}{lightgray}{white} % Empieza en la segunda fila, alterna gris/blanco

#+LATEX: \renewcommand{\arraystretch}{1.35}
#+attr_ipynb: (slideshow . ((slide_type . slide)))
#+CAPTION: Breve resumen sobre los modelos AR(p), MA(q) y ARMA(p,q)
#+ATTR_LATEX: :placement [H]
#+ATTR_LATEX: :environment tabularx :width \textwidth :align |p{3.2cm}|X|X|X|
...
# +LATEX: \rowcolors{0}{}{} % Para resetear después de la tabla

# | Operador \( \pi(B) \)                            | \( \pi(B) = \phi(B)\) (finito)     | \( \pi(B) = \theta^{-1}(B)\) (infinito) | \( \pi(B) = \theta^{-1}(B)\phi(B)\) (infinito) |
# | Operador \( \psi(B) \)                           | \( \psi(B) = \phi^{-1}(B)\) (infinito) | \( \psi(B) = \theta(B)\) (finito) | \( \psi(B) = \phi^{-1}(B)\theta(B)\) (infinito) |
...
# $\phi_1=0.7$ y $\theta_1=-0.8$

#+attr_ipynb: (slideshow . ((slide_type . subslide)))
$$(1 - 0.7\mathsf{B}){X_t}=(1 + 0.8\mathsf{B}){U_t};\quad \phi>0;\;\theta<0$$

#+attr_ipynb: (slideshow . ((slide_type . skip)))
#+BEGIN_SRC jupyter-python :results file :output-dir ./img/lecc06/ :file ACF-arma11R.png :exports code :results silent
ar_params = [1, -0.7]
ma_params = [1,  0.8]
...
[[./img/lecc06/ACF-arma11R.png]]



#+attr_ipynb: (slideshow . ((slide_type . skip)))
#+BEGIN_SRC jupyter-python :results file :output-dir ./img/lecc06/ :file Sim-arma11R.png :exports code :results silent
...
#+attr_ipynb: (slideshow . ((slide_type . skip)))
#+BEGIN_SRC jupyter-python :results file :output-dir ./img/lecc06/ :file ACF-arma11A.png :exports code :results silent
ar_params = [1,  0.7]
ma_params = [1, -0.8]
fig = plot_arma_parametric_diagnostics(ar_params, ma_params, sigma2=1, lags=20)
...
#+attr_html: :width 900px
#+attr_latex: :width 425px
[[./img/lecc06/ACF-arma11A.png]]



#+attr_ipynb: (slideshow . ((slide_type . skip)))
#+BEGIN_SRC jupyter-python :results file :output-dir ./img/lecc06/ :file Sim-arma11A.png :exports code :results silent
fig = plot_arma_analysis(ar_params, ma_params, seed=2026)
...
#+attr_ipynb: (slideshow . ((slide_type . skip)))
#+BEGIN_SRC jupyter-python :results file :output-dir ./img/lecc06/ :file ACF-arma11NVA.png :exports code :results silent
ar_params = [1, -0.4]
ma_params = [1, -0.8]
fig = plot_arma_parametric_diagnostics(ar_params, ma_params, sigma2=1, lags=20)
...
#+attr_html: :width 900px
#+attr_latex: :width 425px
[[./img/lecc06/ACF-arma11NVA.png]]



#+attr_ipynb: (slideshow . ((slide_type . skip)))
#+BEGIN_SRC jupyter-python :results file :output-dir ./img/lecc06/ :file Sim-arma11NVA.png :exports code :results silent
fig = plot_arma_analysis(ar_params, ma_params, seed=2026)
...
#+attr_ipynb: (slideshow . ((slide_type . skip)))
#+BEGIN_SRC jupyter-python :results file :output-dir ./img/lecc06/ :file ACF-arma11RNV.png :exports code :results silent
ar_params = [1,  0.8]
ma_params = [1,  0.4]
fig = plot_arma_parametric_diagnostics(ar_params, ma_params, sigma2=1, lags=20)
...
#+attr_html: :width 900px
#+attr_latex: :width 425px
[[./img/lecc06/ACF-arma11RNV.png]]



#+attr_ipynb: (slideshow . ((slide_type . skip)))
#+BEGIN_SRC jupyter-python :results file :output-dir ./img/lecc06/ :file Sim-arma11RNV.png :exports code :results silent
fig = plot_arma_analysis(ar_params, ma_params, seed=2026)
...
temporal en el horizontal ($x$). Es útil para detectar visualmente:
 ...
 - valores atípicos (/outliers/)
   ...
   + la probabilidad de que una variable normal genere un valor fuera
    ...
    ([[file:./img/lecc03/IBEX35.png][véase gráfico IBEX 35]])
...
#+attr_ipynb: (slideshow . ((slide_type . skip)))
#+BEGIN_SRC jupyter-python :results file :output-dir ./img/lecc06/ :file rangoMedianaPasajeros.png  :exports code :results silent
# Leer los datos
...
#+attr_html: :width 900px
#+attr_latex: :width 425px
[[file:img/lecc06/rangoMedianaPasajeros.png]]
...
# El operador diferencia, $\nabla$, se define como $\nabla = 1 - \mathsf{B}$, donde $\mathsf{B}$ es el operador de retardo.


Un proceso estocástico sin componentes deterministas es $I(0)$ si tiene representación ARMA _estacionaria e invertible_.
...
Un proceso estocástico $\boldsymbol{X}$ /es integrado de orden $d$/, decimos que _es $I(d)$_, si $\;(1-\mathsf{B})^d*\boldsymbol{X}\;$ es $I(0)$.
...
#+attr_ipynb: (slideshow . ((slide_type . skip)))
#+BEGIN_SRC jupyter-python :results file :output-dir ./img/lecc06/ :file diferenciasPasajeros.png  :exports code :results silent
...
#+attr_html: :width 900px
#+attr_latex: :width 425px
[[file:img/lecc06/diferenciasPasajeros.png]]
...
pues el resultado muestra una pauta estacional. Tampoco basta tomar solo una diferencia estacional:
$$\nabla_{12}(\boldsymbol{y})=(1-\mathsf{B^{12}})*\boldsymbol{y};$$
pues resulta una serie que ``deambula'', i.e., que no es /``estacionaria''/ en media.
Tomar ambas diferencias (una ordinaria y otra estacional):
$$\nabla\nabla_{12}(\boldsymbol{y})=(1-\mathsf{B})*(1-\mathsf{B^{12}})*\boldsymbol{y}$$
arroja una serie que sí parece ser /``estacionaria''/.


#+attr_ipynb: (slideshow . ((slide_type . skip)))
#+BEGIN_SRC jupyter-python  :results file :output-dir ./img/lecc06/ :file diferenciasTemperaturasRetiro :exports code :results silent
...
#+attr_html: :width 900px
#+attr_latex: :width 425px
[[file:img/lecc06/diferenciasTemperaturasRetiro.png]]
...
#+attr_ipynb: (slideshow . ((slide_type . skip)))
#+BEGIN_SRC jupyter-python :results file :output-dir ./img/lecc06/ :file diferenciasPoblacion.png  :exports code :results silent
...
#+attr_ipynb: (slideshow . ((slide_type . notes)))
La serie de población $\boldsymbol{y}$ tiene una clara tendencia creciente (primer gráfico), que desaparece al tomar una diferencia ordinaria, $$\nabla\boldsymbol{y}=(1-\mathsf{B})*\boldsymbol{y}$$ (segundo gráfico). 
Bastar con tomar una primera diferencia de la serie de población para obtener una nueva serie que se asemeja a la realización de un proceso estacionario.
...
Pues que la serie obtenida también es estacionaria, pero ojo, es un grave error tomar más diferencias de las necesarias al modelizar los datos. 
Se debe tomar el mínimo número de transformaciones que arrojen una serie ``estacionaria'' 
(recuerde que decir que una serie temporal es /estacionaria/ es un abuso del lenguaje).
...
#+attr_ipynb: (slideshow . ((slide_type . fragment)))
Es importante tomar el número necesario de diferencias y ni una más. 
Por ello es conveniente realizar contrastes de hipótesis que nos ayuden a tomar la decisión correcta.
...
Habrá una raíz unitaria (será no-estacionario) si $\phi =1.\;$ 
...
\] 
que añade $p$ retardos del regresando como regresores para permitir autocorrelación. 
Por lo demás, el test se calcula de la forma habitual 
(comparando de ratio $t$ de $\widehat{\delta}$ con las tablas a la correspondiente variante del test ADF).
...
Estos test son poco potentes y son frágiles ante incumplimientos (heterocedasticidad o no-normalidad); 
por ello debemos complementarlos con el análisis gráfico.
...
La /k/-ésima autocorrelación muestral simple ($\widehat{\rho_k}$) se define como:
\[
...
Para valorar la significatividad individual de estas autocorrelaciones puede usarse el error estándar asintótico:
$s.e.(\widehat{\rho_k})=1/\sqrt{n}$.

Para contrastar la $H_0:$ */los $k$ primeros retardos son conjuntamente no significativos/* se emplea el test de Ljung-Box 
\[Q(h) = n\left(n+2\right)\sum_{k=1}^h\frac{\hat{\rho}^2_k}{n-k}\sim \chi^2(h-p-q)\]
Usaremos el test de Ljung-Box, para evaluar la bondad de un modelo, contrastando si sus residuos son un proceso de /ruido blanco/ (si sus $h$ primeras autocorrelaciones son conjuntamente nulas).
...
O bien, calculando el /k/-ésimo coeficiente MCO de una autorregresión de orden $k$ 
\[
\widetilde{X}_t =
...
\]
donde $\widetilde{X}_t=X_t-\bar{X}\;$ y donde $\;\widehat{\pi_k}=\widehat{\phi_{kk}}$.
...
- Una conﬁguración reconocible de valores estadísticamente no significativos en la ACF o en la PACF muestrales, sí puede ser relevante.
...
* Función de autocovarianzas para un ARMA(p,q)
   :PROPERTIES:
   :metadata: (slideshow . ((slide_type . slide)))
   :ID:       315309b8-425e-48eb-a97d-083940275fe0
   :END:
...
Donde hemos usando $\eqref{eqnLadoIzquierdoYW}$ y hemos renombrado
...
\begin{equation}
  ...
  \end{cases}
\end{equation}
(Donde (5-Lecc 3) se refiere a la [[https://mbujosab.github.io/Econometria-Aplicada/Lecciones-pdf/Lecc03.pdf#subsection.5.2][última ecuación de la Lección 3]])
...
Dividiendo por $\gamma_0$ (y recordando que $\rho_0=1$ en la secuencia $\boldsymbol{\rho}$):
...
#jupyter nbconvert --execute --inplace Lecc06.ipynb
#+END_SRC

#+BEGIN_SRC sh :results silent :eval yes
#jupyter nbconvert --config ../mycfg-GitHubPages.py --to slides --reveal-prefix "https://unpkg.com/reveal.js@5.2.1" --execute Lecc06.ipynb
#+END_SRC
