...
#+begin_abstract
Hoy veremos la ACF, la PACF y la densidad espectral para el caso de los procesos autorregresivos (AR). Estos procesos se caracterizan por tener infinitos coeficientes de autocorrelación no nulos. Presentan una memoria relativamente larga, ya que el valor actual está correlado con todos los valores previos, aunque con coeficientes que disminuyen suficientemente rápido. Esta propiedad permite representar un proceso AR estacionario como una función lineal de todas las innovaciones generadoras, con pesos que tienden a cero a medida que aumenta el retardo. Veremos que los coeficientes de autocorrelación deben verificar las Ecuaciones de Yule-Walker.
#+end_abstract
...
#+attr_ipynb: (slideshow . ((slide_type . subslide)))
- Los procesos AR tienen infinitos coeficientes de autocorrelación no nulos.
...
- Se pueden representar como una función lineal de todas las innovaciones generadoras --- MA($\infty$).
- Los coeficientes de autocorrelación deben verificar las Ecuaciones de Yule-Walker.
