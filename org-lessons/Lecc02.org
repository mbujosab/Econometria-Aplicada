...
- ([[https://mbujosab.github.io/Econometria-Aplicada/Transparencias/Lecc02.slides.html][slides]]) --- ([[https://mbujosab.github.io/Econometria-Aplicada/Lecciones-html/Lecc02.html][html]]) --- ([[https://mbujosab.github.io/Econometria-Aplicada/Lecciones-pdf/Lecc02.pdf][pdf]]) --- ([[https://mybinder.org/v2/gh/mbujosab/Econometria-Aplicada/gh-pages?labpath=CuadernosElectronicos/Lecc02.ipynb][mybinder]]) 
  + implementacion series formales: ([[https://mbujosab.github.io/Econometria-Aplicada/Transparencias/implementacion_series_formales.slides.html][slides]]) --- ([[https://mbujosab.github.io/Econometria-Aplicada/Lecciones-html/src/implementacion_series_formales.html][html]]) --- ([[https://mybinder.org/v2/gh/mbujosab/Econometria-Aplicada/gh-pages?labpath=CuadernosElectronicos/implementacion_series_formales.ipynb][mybinder]]) 
...
Como estas secuencias son infinitas en ambas direcciones, _no hay un primer elemento._ 
...
En una sucesión $\boldsymbol{c}$ _no nula_ llamamos:
...
- Absolutamente sumable ($\ell^1$) :: si $\quad\sum_{t=-\infty}^\infty |c_t| < \infty$ src_latex{(véase Cor. \ref{cor:SumabilidadYConvergenciaAbsoluta}).} {{{results(@@latex:(véase Cor. \ref{cor:SumabilidadYConvergenciaAbsoluta}).@@)}}} 
- De cuadrado sumable ($\ell^2$) ::   si $\quad\sum_{t=-\infty}^\infty c_t^2 < \infty$.
Toda sucesión absolutamente sumable también es de cuadrado sumable, $\ell^1\subset \ell^2$ src_latex{(Prop. \ref{prop:l1dentrodel2}).} {{{results(@@latex:(Prop. \ref{prop:l1dentrodel2}).@@)}}} 
...
$R(a_j)=a_{-j}$:
$$R\big(\boldsymbol{a}(z)\big)=\boldsymbol{a}(z^{-1});$$ 
se puede demostrar que para toda secuencia con final $\boldsymbol{a}$
...
([[https://mbujosab.github.io/Econometria-Aplicada/Transparencias/implementacion_series_formales.slides.html][Implementación en Python]] ---enlace al [[https://mybinder.org/v2/gh/mbujosab/Econometria-Aplicada/gh-pages?labpath=CuadernosElectronicos/implementacion_series_formales.ipynb][Notebook]]).
...
donde $c$ es un número real y cada ${\displaystyle \boldsymbol{p}_{i}}$ es un polinomio cuyo coeficiente para $z^0$ es $1$, y de grado a lo sumo dos, con coeficientes reales. 
...
#+attr_ipynb: (slideshow . ((slide_type . subslide)))
Podemos factorizar un polinomio $\boldsymbol{a}$ _sin raíces de módulo $1$_ como: 
\(
...
es _la inversa_ de $\boldsymbol{a}$ en $\ell^1$.
...
El [[https://en.wikipedia.org/wiki/Rational_function][/cuerpo de fracciones de polinomios/]]:
...
#+BEGIN_EXPORT latex
\emph{Toda fracción de sucesiones con grado y cogrado finitos (con principio y final) pertenece al cuerpo de fracciones de polinomios},
pues  toda sucesión con grado $-k$ y cogrado es de la forma $\boldsymbol{p}*(z^k)^{-\triangleright}$, 
donde $\boldsymbol{p}$ es un polinomio.
#+END_EXPORT
...
#+BEGIN_EXPORT latex
Los manuales de series temporales que conozco no presentan un tratamiento sistemático de las secuencias de números; pese a ser una herramienta fundamental en la modelización ARIMA. Esta sección tiene por objeto servir de referencia donde encontrar la demostración formal de un conjunto de resultados que son aplicados en los manuales de series temporales. 
#+END_EXPORT
