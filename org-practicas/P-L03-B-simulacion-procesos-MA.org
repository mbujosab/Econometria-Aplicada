...
3. Usar dicha función en un bucle para comprobar empíricamente los procesos MA son estacionarios.
...
matrix theta = SimuladorMA( {1, 1.8, 0.9} )
...
#+BEGIN_CENTER
#+CAPTION: Figura con la simulación de 200 MA con el mismo modelo
#+attr_org: :width 500
#+ATTR_LATEX: :width 0.45\textwidth :center t
#+NAME: fig:MuchosMA
...
Como podemos ver la media y varianza parecen similares. Pero no se ve muy bien. Mejor, hagamos un corte en los instantes \(t = 1, 70, 140\) y $200$ para crear un histograma de los valores tomados por las 300 simulaciones en esos instantes.
...
Este resultado coincide con la [[fig:MuchosMA]], donde se observa que las realizaciones del proceso tienen un comportamiento homogéneo a lo largo del tiempo (no podía ser de otro modo, pues los procesos MA son estacionarios).
