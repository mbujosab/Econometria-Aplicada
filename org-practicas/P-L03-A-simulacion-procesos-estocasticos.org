...
#+Title: Lección 3 --- Simulación procesos estocásticos
...
#+PROPERTY: P-L03-A-simulacion-procesos-estocasticos
#+BEGIN_SRC sh  :exports none  :results silent :var practica="P-L03-A-simulacion-procesos-estocasticos"
rm -fr $(pwd)/$practica
...
#+begin_src gretl :tangle guiones/P-L03-A-simulacion-procesos-estocasticos.inp :exports none :eval no
...
* Objetivo de la práctica
   ...
   | Guión: | [[https://mbujosab.github.io/Econometria-Aplicada/Practicas-html/guiones/P-L03-A-simulacion-procesos-estocasticos.inp][P-L03-A-simulacion-procesos-estocasticos.inp]] |

*Objetivo*

1. Simular algunos procesos estocásticos con Gretl con procedimientos rudimentarios
2. Comparar algunos procesos estocásticos estacionarios con otros no estacionarios

* Actividad 1 - Fijar el tamaño y fechas de la muestra

Para simular series temporales es necesario establecer el tamaño de la muestra y a qué fechas corresponderán los datos simulados.
...
- En una tercera ventana emergente indicaremos la fecha de inicio de la muestra (por ejemplo frecuencia (por ejemplo el primer trimestre de 1960) y pulsaremos en */~Adelante~/*.
...
#+begin_src gretl :tangle ./guiones/P-L03-A-simulacion-procesos-estocasticos.inp :eval no
...
Generemos un proceso de ruido blanco con distribución Normal(0,1):
...
#+BEGIN_SRC R :tangle ./guiones/P-L03-A-simulacion-procesos-estocasticos.inp :eval no :exports none :noweb yes
...
pero en el script hemos empleado una función dedicada específicamente a la simulación de datos con distribución normal $N(\mu,\sigma)$. Teclee ~help normal~ para consultar la documentación).
#+latex: }
...
#+end_src
(en el script [[https://mbujosab.github.io/Econometria-Aplicada/Practicas-html/guiones/P-L03-A-simulaciones.inp][P-L03-A-simulaciones.inp]] se hace de manera distinta de manera que los estadísticos se guarden en un fichero y el gráfico en otro (sin generar ningún icono. Dichos ficheros los he utilizado para mostrar los resultados a continuación).
#+latex: }
...
#+BEGIN_SRC R :tangle ./guiones/P-L03-A-simulacion-procesos-estocasticos.inp :eval no :exports none
...
#+latex: {\footnotesize
#+include: ./P-L03-A-simulacion-procesos-estocasticos/EstadisticosWN.txt example
#+latex: }
...
[[file:P-L03-A-simulacion-procesos-estocasticos/WN.png]]
#+END_CENTER


Fíjese que la media y la desviación típica muestrales no coinciden con los parámetros indicados al generar los números aleatorios pero, como dichos estadísticos son unos buenos estimadores de los momentos teóricos de la distribución con la que se han generado los datos, sus valores están muy próximos a los parámetros indicados en la simulación. 

Como la semilla que he usado es diferente de la que usted está usando, los números (pseudo)aleatorios que usted ha obtenido serán distintos; consecuentemente los estadísticos también serán distintos, así como su gráfico (pero lo que usted obtenga se parecerá bastante a lo mostrado aquí).
...
#+latex: {\footnotesize
#+include: ./P-L03-A-simulacion-procesos-estocasticos/EstadisticosWN-1992.txt example
#+latex: }
...
#+latex: {\footnotesize
#+include: ./P-L03-A-simulacion-procesos-estocasticos/EstadisticosWN-2034.txt example
#+latex: }


* Actividad 2 - Generar procesos de media móvil

Simulemos un proceso MA(1) a partir de la simulación de ruido blanco anterior; por ejemplo, simulemos el proceso
\[
 X_t = U_t - \theta_1 U_{t-1},\quad \text{con } U_t\sim N(0,1).
\]
con $\theta_1 = 0.8$.
#+latex: \medskip
...
- En la ventana emergente marcamos escribimos =X = WN + 0.8*WN(-1)= 
...
series X = WN + 0.8*WN(-1)
  #+end_src
  donde =WN(-1)= es la serie =WN= retrasada un periodo.
  #+latex: }
...
#+attr_org: :width 500
#+ATTR_LATEX: :width 0.55\textwidth :center
[[file:P-L03-A-simulacion-procesos-estocasticos/MA1.png]]
#+END_CENTER

#+latex: {\footnotesize
#+include: ./P-L03-A-simulacion-procesos-estocasticos/EstadisticosMA1.txt example
#+latex: }
...
#+latex: {\footnotesize
#+include: ./P-L03-A-simulacion-procesos-estocasticos/EstadisticosMA1-1992.txt example
#+latex: }
...
#+latex: {\footnotesize
#+include: ./P-L03-A-simulacion-procesos-estocasticos/EstadisticosMA1-2034.txt example
#+latex: }
...
- Genere más ejemplos con parámetros distintos.

Genere los gráficos y los estadísticos. Compare los estadísticos de las distintas sub-muestras.


* Código completo de la práctica

   | Guión completo: | [[https://mbujosab.github.io/Econometria-Aplicada/Practicas-html/guiones/P-L03-A-simulacion-procesos-estocasticos.inp][P-L03-A-simulacion-procesos-estocasticos.inp]] |


# +LATEX: \clearpage
#+latex: {\newpage % vspace{10pt}
#+latex: \noindent
*Código completo de la práctica* ~P-L03-A-simulaciones.inp~
#+latex: \vspace{10pt}
\lstinputlisting{guiones/P-L03-A-simulacion-procesos-estocasticos.inp}
#+LATEX: \clearpage }
